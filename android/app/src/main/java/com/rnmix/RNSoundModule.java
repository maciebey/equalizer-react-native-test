...
public class RNSoundModule extends ReactContextBaseJavaModule implements AudioManager.OnAudioFocusChangeListener {
  ...
  Map<Double, Equalizer> eqPool = new HashMap<>();
  ...
  @ReactMethod
  public void bands(final Double key, final Callback callback) {
    Equalizer eq = this.eqPool.get(key);
    short numOfBands = eq.getNumberOfBands();
    short bandRange[] = eq.getBandLevelRange();
    short defaultBandLevel = eq.getBandLevel((short)0);

    WritableMap m = Arguments.createMap();;
    m.putDouble("key", key);
    m.putDouble("numOfBands", numOfBands);
    m.putDouble("bandRangeMin", bandRange[0]);
    m.putDouble("bandRangeMax", bandRange[1]);
    m.putDouble("defaultBandLevel", defaultBandLevel);
    callback.invoke(m);
  }

  @ReactMethod
  public void setBandLevel(final Double key, final int bandNumber, final int change,final Callback callback) {
    Equalizer eq = this.eqPool.get(key);
    eq.setEnabled(true);
    short numOfBands = eq.getNumberOfBands();
    short bandRange[] = eq.getBandLevelRange();
    short currentBandLevel = eq.getBandLevel((short)bandNumber);

    short newLevel = (short) (currentBandLevel + change);
    if (newLevel > bandRange[1]) {
      newLevel = bandRange[1];
    }

    eq.setBandLevel((short)bandNumber, newLevel);

    // eq.setBandLevel((short)0, newLevel);
    // eq.setBandLevel((short)1, newLevel);
    // eq.setBandLevel((short)2, newLevel);
    // eq.setBandLevel((short)3, newLevel);
    // eq.setBandLevel((short)4, newLevel);

    callback.invoke((int)eq.getBandLevel((short)bandNumber));
  }
